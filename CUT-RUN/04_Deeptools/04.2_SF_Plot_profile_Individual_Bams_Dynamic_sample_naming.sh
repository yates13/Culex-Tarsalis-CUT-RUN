...
################################################################################
# STEP 4.2: Profile plots of ALL replicates for the SF group, one plot per
#           target and day
#
...
#   another, copy this one and change SF to the new group in: the job name, the
#   "SF-*" filter, the SF_SAMPLES/MATRIX_SF names, the output file name, and the
...
#   3. sbatch 04.2_SF_Plot_profile_Individual_Bams_Dynamic_sample_naming.sh
#
# NOT AN ARRAY JOB
#   There is no array size to change. One job loops over every target/day. The
#   sample lists are built from the bigWig files on disk, so new SF replicates
#   are picked up automatically.
#
# HOW SAMPLES ARE CHOSEN
#   Sample names look like <GROUP>-<TARGET>-<DAY>-R<n> (e.g. SF-AC-D1-R1). For
#   each target/day, every bigWig named *-<TARGET>-<DAY>-*_inputsubtracted.bw is
#   found, then those starting with "SF-" are kept. A target/day with no SF
#   samples is skipped with a message and the job still finishes.
#
# LIMIT: 8 REPLICATES PER PLOT
#   COLOR_PALETTE has 8 colors, and each SF sample needs one. If a target/day
#   has more than 8 SF samples, plotProfile gets fewer colors than samples and
...
#   ls <OUTDIR>/*_SF_profile.png     # 6 plots
#   grep "SF samples:" out_log/*.log # which samples went into each plot
...
# Plot settings
# ----------------------------
# One color per SF replicate, used in order (max 8 replicates per plot)
